...
function CurvedDivider() {
  return (
    <div className="pointer-events-none absolute inset-y-10 left-1/2 hidden w-40 -translate-x-1/2 xl:block">
      <svg viewBox="0 0 160 720" className="h-full w-full -scale-x-100" preserveAspectRatio="none" aria-hidden="true">
        <path
    ...
    <section className="auth-fade-in relative flex min-h-[320px] flex-col justify-between overflow-hidden rounded-xl border border-sky-200 bg-gradient-to-br from-sky-600 via-sky-500 to-cyan-400 px-8 py-10 text-white shadow-[0_24px_64px_rgba(59,130,246,0.24)] lg:px-10">
      ...
      <div className="relative z-10 flex items-center gap-3">
        ...
        <div>
          ...
          <h1 className="mt-2 text-3xl font-semibold tracking-[0.22em]">Register Workspace</h1>
        </div>
      </div>
      ...
      <div className="relative z-10 max-w-md space-y-6">
        ...
        <div>
          <p className="text-sm uppercase tracking-[0.4em] text-sky-100/85">Join SIMS</p>
          <h2 className="mt-4 text-4xl font-semibold leading-tight">
            Create a clean, secure entry point for your inventory team.
          </h2>
        </div>

        <p className="max-w-sm text-sm leading-7 text-sky-50/90">
          Register a new account to start managing spare parts, inbound stock,
          outbound usage, and daily reporting in one place.
        </p>
      </div>
    </section>
  );
...
function RegisterCard({ form, setForm, loading, error, success, handleSubmit }) {
  return (
    <section className="auth-fade-in relative z-10 w-full max-w-xl rounded-xl border border-sky-100 bg-white/95 p-8 shadow-[0_24px_56px_rgba(30,58,95,0.08)] backdrop-blur xl:p-10">
      <div className="mb-8 flex items-start justify-between gap-4">
        <div>
          <p className="text-xs uppercase tracking-[0.35em] text-sky-500">Register</p>
          <h2 className="mt-3 text-3xl font-semibold text-slate-800">Create your SIMS account</h2>
          <p className="mt-3 max-w-md text-sm leading-6 text-slate-500">
            This form still submits through your existing backend registration
            API, with the same auth context flow you already had.
          </p>
        </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <button
          disabled={loading}
          className="inline-flex w-full items-center justify-center gap-2 rounded-lg bg-sky-600 px-5 py-3.5 text-sm font-semibold text-white transition duration-200 hover:bg-sky-700 disabled:cursor-not-allowed disabled:opacity-60"
        >
      ...
      <p className="mt-6 text-sm text-slate-500">
        ...
        <Link
          to="/login"
          className="inline-flex items-center gap-2 font-semibold text-sky-700 underline decoration-sky-200 underline-offset-4 transition hover:decoration-sky-500"
        >
          <ArrowLeft size={14} />
          Back to login
        </Link>
...
const Register = () => {
  ...
  return (
    <div className="relative min-h-screen overflow-hidden bg-white">
      <div className="pointer-events-none absolute inset-0 bg-[radial-gradient(circle_at_top_right,_rgba(96,165,250,0.18),_transparent_34%),radial-gradient(circle_at_bottom_left,_rgba(125,211,252,0.16),_transparent_28%)] bg-fixed" />
      <div className="pointer-events-none absolute left-[-8rem] bottom-16 h-80 w-80 rounded-full bg-sky-200/40 blur-3xl" />
      <div className="pointer-events-none absolute right-[-6rem] top-24 h-72 w-72 rounded-full bg-cyan-100/60 blur-3xl" />

      <CurvedDivider />

      <div className="relative mx-auto grid min-h-screen max-w-7xl grid-cols-1 items-center gap-8 px-4 py-8 md:px-8 xl:grid-cols-[minmax(0,1.05fr)_minmax(0,0.95fr)] xl:gap-16 xl:px-12">
        <BrandPanel />
        <RegisterCard
          form={form}
          setForm={setForm}
